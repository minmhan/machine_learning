...
y = eye(num_labels)(y,:); % get 5000 x 10 label matrix
tmp = 0;

%for c=1:m % 1 to 5000 
%	for k=1:num_labels % 1 to 10
%		a2 = sigmoid(X(c,:)*Theta1');
%		a2 = [ones(1) a2];
%		a3 = sigmoid(a2*Theta2');
%		tmp = tmp + ((-y(c,:).*log(a3)) - ((1-y(c,:)).*log(1-a3)));
%	end
%end
%J = (1/m) * tmp;
...
a2 = [ones(m,1) a2];
a3 = sigmoid(a2*Theta2');
J_tmp = (1/m) * sum(sum(-y.*log(a3) - (1-y).*log(1-a3)));

%tmp1 = input_layer_size + 1;
%tmp2 = hidden_layer_size + 1;
...
delta3 = zeros(1,num_labels); %1x10
for t=1:m
	a1 = X(t,:); %1 x 401
	z2 = a1*Theta1'; % 1 x 25
	...
	%disp(a3);
	for k=1:num_labels
		%delta3(k)=a3(k)-(y(m) == num_labels);
		delta3(k) = a3(k) - y(t,k);
		%disp(y(m,k));
	end
	%disp(delta3);
	delta2 = delta3 * Theta2(:,2:end).*sigmoidGradient(z2); % 1 x 25
